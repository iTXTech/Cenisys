...
public:
    ...
    BOOST_CONSTEXPR basic_string_ref(const charT *str,
                                     size_type len) BOOST_NOEXCEPT : ptr_(str),
                                                                     len_(len)
    {
    }
    ...
    size_type rfind(basic_string_ref s) const
    {
        const_reverse_iterator iter = std::search(
            this->crbegin(), this->crend(), s.crbegin(), s.crend(), traits::eq);
        return iter == this->crend()
                   ? npos
                   : (std::distance(iter, this->crend()) - s.size());
    }

    size_type rfind(charT c) const
    {
        const_reverse_iterator iter =
            std::find_if(this->crbegin(), this->crend(),
                         detail::string_ref_traits_eq<charT, traits>(c));
        return iter == this->crend()
                   ? npos
                   : (this->size() - 1 - std::distance(this->crbegin(), iter));
    }
    ...
    {
        const_reverse_iterator iter =
            find_not_of(this->crbegin(), this->crend(), s);
        return iter == this->crend()
                   ? npos
                   : (this->size() - 1 - std::distance(this->crbegin(), iter));
    }
...
inline void sr_insert_fill_chars(std::basic_ostream<charT, traits> &os,
                                 std::size_t n)
{
    enum
    {
...
template <class charT, class traits>
void sr_insert_aligned(std::basic_ostream<charT, traits> &os,
